...
class PostReplyChildPagingSource(private val postId: String): PagingSource<QuerySnapshot, Post.ReplyPost>() {
    ...
    override suspend fun load(params: LoadParams<QuerySnapshot>): LoadResult<QuerySnapshot, Post.ReplyPost> {
        return try {
            val currentPage = params.key ?: db.collection("reply_post_child")
            ...
            val nextPage = db.collection("reply_post_child")
                .whereEqualTo("postId", postId)
                .orderBy("created_at")
            ...
            Log.d("PostReplyChildPagingSource", replyPostParent.toString())

            LoadResult.Page(
                data = replyPostParent,
                prevKey = null,
                nextKey = if (nextPage.isEmpty) null else nextPage
            )
        }catch (e: Exception) {
            Log.d("PostReplyChildPagingSource", e.message.toString())
            LoadResult.Error(e)
        }
    }
}